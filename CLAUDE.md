...
**Top-down scaffolding enforcement:** The system prevents fragment accumulation — building isolated pieces that never connect into a working whole. The architect agent (`groom --architect`, also runs in auto-groom) detects fragment risk signals (stub entry points, mock-only infrastructure, disconnected modules) and creates scaffolding backlog items with `must` priority and `dependsOn` chains. The developer agent is instructed to wire code to real entry points. The validator flags disconnected code as advisory notes. The product-owner watches for feature priorities outrunning the skeleton during grooming.
